...
$(document).ready(function(){
	...
	check_payment_interval = setInterval(check_payment_history,30000);
	
});

function dateDiff(date,now_time) {
	console.log('date diff called');
 	// var d2 = Date.now();
...
function dateTimer() {
	console.log('date timer called');
 	if($("#expire_on").val() != ''){
 		console.log('date timer in');
 		console.log('expired on >>> ',$("#expire_on").val());
...
function check_payment_history(){
	...
	$.ajax({
		url:'index.php?route=extension/module/coinremitter_invoice/payment_history',
		type:'post',
		data:{address},
		success:function(res){
			if(res.flag == 1){
				...
				if(res.is_success == 1){
					...
					window.location.replace("index.php?route=extension/module/coinremitter_invoice/success&order_id=" + enc_order_id);
					console.log(res);
				}else{
					//update invoice
					...
					if(res_data.nopayment == 1){
						if($('#paymentStatus').is(':empty')){
							...
							$("#timerStatus").append("<span>Payment Status : " + res_data.order_status + "</span>");
		                  	clearInterval(time_interval);
		                  	$("#paymentStatus").append("<span>Awaiting Payment</span>");
...
function funExpire(enc_order_id) {
 	window.location.replace("index.php?route=extension/module/coinremitter_invoice/expired&order_id=" + enc_order_id);
}

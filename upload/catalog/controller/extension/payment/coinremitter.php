...
class ControllerExtensionPaymentCoinremitter extends Controller {
	...
	public function __construct($registry){
		...
		$this->obj_curl = Coinremitter::get_instance($this->registry);
	...
	public function index() {
		...
		$add_column_if_not_exists = $this->model_extension_payment_coinremitter->checkIsValidColumn();
		...
		return $this->load->view('extension/payment/coinremitter',$validate_coin);
	}
				...
				if($wallet_info){
                    ...
                    if(!empty($address_res) && isset($address_res['flag']) && $address_res['flag'] == 1){
                    	...
                    	if(!empty($fiat_to_crypto_res) && isset($fiat_to_crypto_res['flag']) && $fiat_to_crypto_res['flag'] == 1){
		                        ...
		                        $invoice_url = $this->url->link('extension/module/coinremitter_invoice/detail&order_id='.$enc_order_id);

		                    	/*** Update order history status to pending, add comment  ***/
		                        $comments = 'View order <a href="'.$invoice_url.'">#' . $orderId . '</a> ';
		                        $is_customer_notified = true;
		                        $this->model_checkout_order->addOrderHistory($orderId, 1, $comments, $is_customer_notified); 
		                        													// 1 = Pending
			...
			if(!empty($post)){
				...
				if(isset($post['invoice_id']) && $post['invoice_id'] != ''){
					...
					if($wallet_info){
						...
						$order_detail = $this->model_extension_payment_coinremitter->getInvoiceOrder($invoice_id);
							...
							if($order_detail['payment_status'] != 'paid' && $order_detail['payment_status'] != 'over paid'){
								...
								if(!empty($invoice) && $invoice['flag'] ==1){
									...
									if($invoice_data['status_code'] == 1 || $invoice_data['status_code'] == 3){
				                        ...
				                        $this->model_extension_payment_coinremitter->updateInvoiceOrder($id,$order_data);
				                        ...
				                        if(empty($invoice_data['payment_history'])){
					                        ...
					                        $this->model_extension_payment_coinremitter->updateInvoicePaymentStatus($id,$payment_data);
					                        ...
					                        $this->load->model('checkout/order');
					                        $is_customer_notified = true;
					                        $this->model_checkout_order->addOrderHistory($orderId, 7, $comments, $is_customer_notified);  // 7 = Canceled
				                        }
				                        $json['flag'] = 1;
				                        $json['msg'] = 'Notified Successfully';
				                    }else{
		...
		if($this->request->server['REQUEST_METHOD'] == 'POST'){
			...
			if(!empty($post)){
				...
				if($post['type'] == 'receive'){
					...
					$this->load->model('extension/payment/coinremitter');
					$order_info = $this->model_extension_payment_coinremitter->getOrderByAddress($address);
													...
													for ($i=0; $i < count($getTrxByAddData); $i++) { 
														...
														if(isset($trx['type']) && $trx['type'] == 'receive'){
															...
															if(empty($webhook_info)){
																...
																$inserted_id =$this->model_extension_payment_coinremitter->addWebhook($insert_arr);
																if($inserted_id > 0){
																	$status = 'expired';
																	$update_arr = array('payment_status' => $status);
																	$this->model_extension_payment_coinremitter->updateOrder($orderId,$update_arr);
																	$update_arr = array('status' => ucfirst($status));
																	$this->model_extension_payment_coinremitter->updatePaymentStatus($orderId,$update_arr);
									...
									if(!empty($wallet_info)){
										...
										if(!empty($getTransaction) && isset($getTransaction['flag']) && $getTransaction['flag'] == 1){
											...
											if(isset($transaction['type']) && $transaction['type'] == 'receive'){
												...
												$webhook_info = $this->model_extension_payment_coinremitter->getWebhook($transaction['id']);
												if(empty($webhook_info)){
													//insert record
													$insert_arr = array(
												...
												$webhook_res = $this->model_extension_payment_coinremitter->getSpecificWebhookTrxByAddress($address);
												...
												if(!empty($webhook_res)){
													...
													foreach ($webhook_res as $webhook) {
														...
														if(!empty($getTransactionRes) && isset($getTransactionRes['flag']) && $getTransactionRes['flag'] == 1){
															...
															$this->model_extension_payment_coinremitter->updateTrxConfirmation($webhook['transaction_id'],$update_confirmation);
														}
													}
												}
	...
	private function add_order_success_history($orderId){
		...
		if(!empty($order_cart)){ //&& $order_cart['order_status'] == 'Pending'
			...
			if(!empty($order_info) && (strtolower($order_info['payment_status']) == 'paid' || strtolower($order_info['payment_status']) == 'over paid' || strtolower($order_info['payment_status']) == 'under paid' || strtolower($order_info['payment_status']) == 'expired')){
				...
				$getWebhookByAddressRes = $this->model_extension_payment_coinremitter->getWebhookByAddress($order_info['address']);	

				if(!empty($getWebhookByAddressRes)){

					/*** Get sum of paid amount of all transations which have 3 or more than 3 confirmtions  ***/
					$total_paid_res = $this->model_extension_payment_coinremitter->getTotalPaidAmountByAddress($order_info['address']);
		            ...
		            $order_url = $this->url->link('extension/module/coinremitter_invoice/detail&order_id='.$enc_order_id);
					...
					if($order_cart['order_status'] == 'Pending'){
						if($order_info['payment_status'] == 'over paid' || $order_info['payment_status'] == 'paid'){
							...
							$this->model_checkout_order->addOrderHistory($orderId,$status, $comments);  // 5 = Complete
						}else{
							$this->model_checkout_order->addOrderHistory($orderId, 1, $comments);  // 1 = pending
						}
					}else{
						$status_id = $this->db->query("SELECT *  FROM " . DB_PREFIX . "order_status where name LIKE '%" . $order_cart['order_status'] . "%'");
											
						$this->model_checkout_order->addOrderHistory($orderId, $status_id->row['order_status_id'], $comments);
					}
				}
			}
		}

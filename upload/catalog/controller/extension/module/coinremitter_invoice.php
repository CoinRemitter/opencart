<?php
class ControllerExtensionModuleCoinremitterInvoice extends Controller {
	...
	public function detail() {
		...
		$coinremitter_order_info = $this->model_extension_payment_coinremitter->getOrder($order_id);
		...
		if ($order_info) {
			...
			$this->document->addStyle('catalog/view/javascript/coinremitter/css/coinremitter_invoice.css');
			$this->document->addScript('catalog/view/javascript/coinremitter/js/coinremitter_invoice.js?v=6');
			...
			$this->response->setOutput($this->load->view('extension/module/coinremitter_invoice', $data));
		}else{
			return new Action('error/not_found');
		}
		...
		if(isset($this->request->post['address']) && $this->request->post['address'] != ''){
			...
			if(!empty($order_info)){
				...
				if($isShowDataFromCoinremitterWebhookTable == 1){
					...
					$getWebhookByAddressRes = $this->model_extension_payment_coinremitter->getWebhookByAddress($address);
						...
						if(!empty($wallet_info)){
							...
							if(!empty($getTransactionByAddressRes) && isset($getTransactionByAddressRes['flag']) && $getTransactionByAddressRes['flag'] == 1){
								...
								//update payment_status,status as expired
									...
									return new Action('checkout/failure');	
								}else{									
									$getTrxByAddData = $getTransactionByAddressRes['data'];
									for ($i=0; $i < count($getTrxByAddData); $i++) { 
	...
	public function success(){
		...
		if(strtolower($order_info['payment_status']) == 'paid' || strtolower($order_info['payment_status']) == 'over paid'){
			...
			return new Action('checkout/success');
		}else{
			return new Action('error/not_found');	
		}
		// if($order_cart['order_status'] == 'Pending' || $order_cart['order_status'] == 'Complete'){
